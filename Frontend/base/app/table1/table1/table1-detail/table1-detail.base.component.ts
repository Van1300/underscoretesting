 ...
 queryParams:any ={};
  ...
  "children" : [ {
    ...
    "children" : [ {
      "visibility" : "show",
      "buttonStyle" : "curved",
      "methodName" : "triggerWebserviceOpenflowunderscoreFlowOpenflow",
      "label" : "OPENFLOW",
      "type" : "button",
      "outline" : true,
      "buttonType" : "icon_on_left",
      "service" : {
        "sid" : "e3e19253-87f9-43dc-afe2-2143767e4d3d",
        "name" : "underscoreFlowOpenflow",
        "tableName" : "table1",
        "tableId" : "e097b516-55e4-4f73-a9b1-febca35eb73f"
      },
      "showOn" : "both",
      "enableOnlyIfRecordSelected" : false,
      "buttonId" : "OpenflowbuttonId0",
      "buttonEnabled" : "yes",
...
public location = inject(Location);
	...
	field1: new UntypedFormControl('',[Validators.required]),
});
	...
	this.table1Service.underscoreFlowOpenflow(params).subscribe((res:any)=>{
 ...
 onChangelog() {
    ...
    if (this.checkValidations()) {
      ...
      if (Object.keys(splittedData.files).length > 0) {
        ...
        const saveSubscription = this.table1Service[method](requestedObj, actionName).subscribe((res: Table1Base) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any,actionName?:string): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.table1Service.create(splittedData.data,actionName).subscribe(
        createdData => {
    ...
    const subject$ = new Subject();    
      const updateSubscription = this.deleteFiles(splittedData).subscribe((responseData: any) => {
        this.saveFiles(responseData).subscribe((dataToUpdate: any) => {
          ...
          this.table1Service.update(dataToUpdate.data,actionName).subscribe(
            res => {
              subject$.next(res);
              subject$.complete();
            },
 ...
 getRoleShortName(currentUserData:any){
  ...
  if (environment.prototype && this.id) {
    ...
    const params = {
      ...
      const dataSubscription = combineLatest(this.table1Service.getById(params), this.appBaseService.getWorkFlowConfig(wfParams)).subscribe((res: any) => {
          this.data = res[0] || {};
...
workflowInformation.step = workflowDataForNewRec.step || '';
	...
	triggerWebserviceOpenflowunderscoreFlowOpenflow() {
    const params = this.getInputParams();
    this.table1Service.underscoreFlowOpenflow(params).subscribe({
