...
/// workflow - starts 
    ...
    config.colConfig?.filterInputMapping?.map((paramsObj: any) => { 
      ...
      if(value &&fromSearch){
        if(  typeof value == 'object' ){
          queryParams = queryParams.append(paramsObj.lookupField, value[paramsObj.lookupField]);
        } 
        else
  ...
  formatAllowedValues(real_Data: any, uiType: any) {
    if (uiType?.toLowerCase() == 'lookup' && real_Data) {
      ...
      return real_Data ? this.domSanitizer.sanitize(SecurityContext.HTML, (typeof real_Data == 'string' ? this.translateService.instant(real_Data) : (Array.isArray(real_Data) ? real_Data?.map(x => this.translateService.instant(x)) : real_Data))) : ''
    }
  }
}
