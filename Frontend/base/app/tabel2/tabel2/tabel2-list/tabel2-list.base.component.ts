...
export class Tabel2ListBaseComponent{
...
localStorageStateKey = "tabel2-list";
...
queryViewFiltersApplied: boolean = false;
 gridEmptyMsg: string = '';
  ...
  "sortOrder" : "asc",
  "detailPage" : {
    "sid" : "7680c530-c54f-4318-a8b1-5f1f21ca04bd",
    "name" : "tabel2 Detail",
    "url" : "/tabel2/tabel2detail"
  },
  "detailPageNavigation" : "click_of_the_row"
}
	pageViewTitle: string = 'TABEL2_LIST';
  ...
  this.clearFilterValues();
	...
	if (this.tableConfig.detailPage?.url) {
      const value: any = "parentId";
       let property: Exclude<keyof Tabel2ListBaseComponent, ''> = value;
       const methodName: any = "onUpdateChild";
       let action: Exclude<keyof Tabel2ListBaseComponent, ''> = methodName;
 ...
 if(!(this.appUtilBaseService.isEqualIgnoreCase(value,this.filter.quickFilter, [],true))){
  ...
  this.filtersApplied = false;
    ...
    return {
      ...
      ajaxUrl: Tabel2ApiConstants.getDatatableData,
      select: true,
...
this.location.back();
    ...
    if (environment.prototype && this.tableConfig.children?.length > 0) {
      gridSubscription = this.tabel2Service.getProtoTypingData().subscribe((data: any) => {
  ...
  this.onRefresh();
    ...
    if (this.selectedValues.length > 0) {
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          const deleteSubscription = this.tabel2Service.delete(requestedParams).subscribe((res: any) => {
