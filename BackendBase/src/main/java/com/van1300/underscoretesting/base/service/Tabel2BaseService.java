...
public class Tabel2BaseService<T extends Tabel2Base> extends BaseJPAWorkflowBusinessLogic<T>
		implements ITabel2BaseService<T> {
	...
	protected Tabel2PerimeterBaseImpl tabel2PerimeterBaseImpl;
			
	public Tabel2BaseService(Class<T> modelClass) {
		super(modelClass);
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("Tabel2", modelObj.getSid().toString(), "Saved", modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("Tabel2", modelObj.getSid().toString(), "Updated", modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("Tabel2", modelObj.getSid().toString(), "Deleted", modelObj);
	}	
	...
	@Override
	public T createopentab2(Object id, Map<String, Object> additionalInfo) {
		setWorkflowName(blanksName);
		T model = getById(id);
		if(model == null) {
		...
		WorkflowMetaInfo metaInfo = createWorkflowMetaInfo(model, model.getField1().name().toLowerCase(), null,
				createopentab2,MapUtil.readValueAsString(ACTION_COMMENTS, additionalInfo));
		...
		onbeforeCreateopentab2(model,metaInfo);
		onCreateopentab2(model,metaInfo);
		onAfterCreateopentab2(model,metaInfo);
		return model;
	}
	
	@Override
	public void onbeforeCreateopentab2(T model, WorkflowMetaInfo metaInfo){
		
	}
	
	@Override
	public void onCreateopentab2(T model, WorkflowMetaInfo metaInfo){
	...
	@Override
	public void onAfterCreateopentab2(T model, WorkflowMetaInfo metaInfo){
	...
	protected void setWorkflowFieldsInModel(T model, WorkflowMetaInfo metaInfo) {
				model.setField1(Tabel2field1.valueOf(metaInfo.getNextStep().toUpperCase()));
	...
	@Override
	public T getTabel2WorkflowModelBySid(Object id) {
		T model = getById(id);
		if (model != null) {
			ApplicationUserBase userPrivilege = userprivilegeCache.getCurrentUser(false);
			if (userPrivilege == null || StringUtils.isBlank(userPrivilege.getEmail())) {
			...
			if(model.getField1() != null){
				workflowInfo.put(blanksName, new WorkflowBasicInfo(StringUtils.lowerCase(model.getField1().name()),
	...
	public void setPerimeter(IWorkflowPerimeterManager<T> tabel2PerimeterImpl) {
		this.tabel2PerimeterBaseImpl = (Tabel2PerimeterBaseImpl) tabel2PerimeterImpl;
	}
	

	@Override
	protected IWorkflowPerimeterManager<T> getPerimeterManager() {
		return this.tabel2PerimeterBaseImpl;
	} 
}
