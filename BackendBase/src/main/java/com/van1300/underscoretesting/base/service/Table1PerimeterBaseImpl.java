...
import com.van1300.underscoretesting.base.model.constants.Table1ConstantBase;
...
@Component
public abstract class Table1PerimeterBaseImpl<T extends Table1Base> implements IWorkflowPerimeterManager<T>, Table1ConstantBase {
	
	private Logger LOGGER = LoggerFactory.getLogger(Table1PerimeterBaseImpl.class);
	...
	protected void setWriteFields(ApplicationUserBase userBase, AllowedFields allowedFields) {
		...
		if(userBase == null) {
			...
			return;
					...
					if (BooleanUtils.isTrue(userBase.isR1())) {
			String[] readFields = new String[] {FIELD1};
			allowedAccessFields.addAll(Arrays.asList(readFields));
		}
